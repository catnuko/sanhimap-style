...
pub fn interpolatedPropertyDefinitionToJsonExpr(
    ...
    if (property.interpolation == undefined or property.interpolation == .Discrete) {
        var step = std.ArrayList(std.json.Value).init(alloc.get());
        ...
        for (1..property.zoomLevels.len) |i| {
            step.append(.{ .float = property.zoomLevels[i] }) catch unreachable;
            step.append(property.values[i]) catch unreachable;
        }
        return step;
    }
    var interpolation = std.ArrayList(std.json.Value).init(alloc.get());
    ...
    switch (property.interpolation) {
        ...
        .Exponential => {
            ...
            interpolation.append(if (property.exponent) |expo| .{ .float = expo } else .{ .float = 2 }) catch unreachable;
        },
        else => console.panic("interpolation mode '{}' is not supported\n", .{property.interpolation}),
    }
    interpolation.append(.{ .string = "zoom" }) catch unreachable;
    for (0..property.zoomLevels.len) |i| {
        interpolation.append(.{ .float = property.zoomLevels[i] }) catch unreachable;
