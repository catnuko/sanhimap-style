...
fn isSpace(codepoint: u32) bool {
    switch (codepoint) {
        Character.Tab, Character.Lf, Character.Cr, Character.Space => return true,
        else => return false,
    }
}

fn isNumber(codepoint: u32) bool {
    return codepoint >= Character._0 and codepoint <= Character._9;
}

fn isLetter(codepoint: u32) bool {
    return ((codepoint >= Character.a and codepoint <= Character.z) or
        (codepoint >= Character.A and codepoint <= Character.Z));
}

fn isLetterOrNumber(codepoint: u32) bool {
    return isLetter(codepoint) or isNumber(codepoint);
}

fn isIdentChar(codepoint: u32) bool {
    return (isLetterOrNumber(codepoint) or
        codepoint == Character._ or
        codepoint == Character.Dollar or
        codepoint == Character.Dot or
    ...
    fn yylex(self: *Self) Token {
        ...
        switch (ch) {
            ...
            Character.SingleQuote, Character.DoubleQuote => {
                ...
                while (self.m_char and self.m_char != ch) {
            ...
            Character.Amp => {
                ...
                if (isLetter(ch) or
                    ch == Character._ or
                    (ch == Character.Dollar and isIdentChar(self.m_char)))
                {
